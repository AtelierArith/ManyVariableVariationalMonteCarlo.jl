@testitem "rng" begin
    ...
    @testset "RNG reproducibility" begin
        ...
        seq1 = [vmcrand(1) for _ = 1:10]
        initialize_rng!(UInt32(99999), 1)
        seq2 = [vmcrand(1) for _ = 1:10]
        @test seq1 == seq2
    end
    ...
    @testset "RNG statistical properties" begin
        ...
        n_samples = 10000
        samples = [vmcrand(1) for _ = 1:n_samples]
        # Test range
        ...
        int_samples = [vmcrand_int(10, 1) for _ = 1:n_samples]
    ...
    @testset "RNG thread safety" begin
        ...
        for tid = 1:4
            results[tid] = [vmcrand(tid) for _ = 1:100]
        end
    ...
    @testset "RNG performance characteristics" begin
        ...
        time_uniform = @elapsed for _ = 1:n_samples
            vmcrand(1)
        end
        @test time_uniform < 0.1  # Should be very fast
        # Time integer generation
        time_int = @elapsed for _ = 1:n_samples
            vmcrand_int(100, 1)
        end
        @test time_int < 0.1
        # Time boolean generation
        time_bool = @elapsed for _ = 1:n_samples
            vmcrand_bool(0.5, 1)
        end
        @test time_bool < 0.1
    end
