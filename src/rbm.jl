...
function rbm_weight(
    rbm::RBMNetwork{T},
    visible_state::Vector{Int},
    hidden_state::Vector{Int},
) where {T}
    ...
    for i = 1:rbm.n_visible
        for j = 1:rbm.n_hidden
...
"""
function update_rbm_weights_phys!(
    rbm::RBMNetwork{T},
    grad_weights::Matrix{T},
    grad_visible_bias::Vector{T},
    grad_hidden_bias::Vector{T},
...
function set_rbm_parameters!(rbm::RBMNetwork{T}, params::Vector{T}) where {T}
    ...
    expected_size =
        rbm.n_hidden * rbm.n_visible +
        rbm.n_visible +
        rbm.n_hidden +
        rbm.n_hidden * rbm.n_phys_layer
...
function rbm_parameter_count(rbm::RBMNetwork{T}) where {T}
    return rbm.n_hidden * rbm.n_visible +
           rbm.n_visible +
           rbm.n_hidden +
           rbm.n_hidden * rbm.n_phys_layer
end
...
"""
function compute_efficient_gradient!(
    grad::RBMGradient{T},
    rbm::RBMNetwork{T},
    visible_state::Vector{Int},
    hidden_state::Vector{Int},
    phys_state::Vector{T} = T[],
) where {T}
    if !rbm.is_initialized
        throw(ArgumentError("RBM not initialized"))
    end
    ...
    @inbounds for i = 1:rbm.n_visible
        for j = 1:rbm.n_hidden
    ...
    grad.gradient_norm = sqrt(
        sum(abs2, grad.grad_weights) +
        sum(abs2, grad.grad_visible_bias) +
        sum(abs2, grad.grad_hidden_bias) +
        sum(abs2, grad.grad_phys_weights),
    )
    grad.max_gradient = maximum([
        maximum(abs, grad.grad_weights),
        maximum(abs, grad.grad_visible_bias),
        maximum(abs, grad.grad_hidden_bias),
        maximum(abs, grad.grad_phys_weights),
    ])
...
"""
function set_parameter_bounds!(
    vrbm::VariationalRBM{T},
    param_name::String,
    lower::T,
    upper::T,
) where {T}
...
"""
function record_optimization_step!(
    vrbm::VariationalRBM{T},
    energy::Float64,
    gradient_norm::Float64,
) where {T}
...
function get_optimization_statistics(vrbm::VariationalRBM{T}) where {T}
    ...
    if n_steps >= 2
        energy_convergence =
            abs(vrbm.energy_history[end] - vrbm.energy_history[end-1]) /
            abs(vrbm.energy_history[end-1])
    else
    ...
    if n_steps >= 2
        gradient_convergence =
            abs(vrbm.gradient_history[end] - vrbm.gradient_history[end-1]) /
            vrbm.gradient_history[end-1]
    else
...
mutable struct RBMEnsemble{T<:Union{Float64,ComplexF64}}
    ...
    function RBMEnsemble{T}(
        n_networks::Int,
        n_visible::Int,
        n_hidden::Int,
        n_phys::Int = 0,
    ) where {T}
        rbms = [RBMNetwork{T}(n_visible, n_hidden, n_phys) for _ = 1:n_networks]
...
"""
function ensemble_rbm_weight(
    ensemble::RBMEnsemble{T},
    visible_state::Vector{Int},
    hidden_state::Vector{Int},
    phys_state::Vector{T} = T[],
) where {T}
    ...
    for i = 1:ensemble.n_networks
        if phys_state isa Vector{T} && !isempty(phys_state)
            weight =
                rbm_weight_phys(ensemble.rbms[i], visible_state, hidden_state, phys_state)
        else
...
"""
function update_ensemble_weights!(
    ensemble::RBMEnsemble{T},
    performance_scores::Vector{Float64},
) where {T}
