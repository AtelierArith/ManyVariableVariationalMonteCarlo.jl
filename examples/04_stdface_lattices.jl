...
function main()
    ...
    println("\n--- 1D Chain Lattice ---")
    L = 8
    ham_chain, geom_chain, config_chain =
        stdface_chain(L, "Hubbard"; t = 1.0, U = 4.0, V = 0.5)
    ...
    println("\n--- 2D Kagome Lattice ---")
    Lx, Ly = 2, 2
    ham_kagome, geom_kagome, config_kagome =
        stdface_kagome(Lx, Ly, "Hubbard"; t = 1.0, U = 2.0)
    ...
    L = 6
    W = 2
    ham_ladder, geom_ladder, config_ladder =
        stdface_ladder(L, W, "Hubbard"; t = 1.0, t_perp = 0.8, U = 2.0)
    ...
    println(
        "  Site 1 (corner): $(length(neighbors_nn[1])) NN, $(length(neighbors_nnn[1])) NN+NNN",
    )
    println(
        "  Site 5 (center): $(length(neighbors_nn[5])) NN, $(length(neighbors_nnn[5])) NN+NNN",
    )
