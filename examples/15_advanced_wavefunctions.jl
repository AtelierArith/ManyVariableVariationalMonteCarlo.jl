...
function main()
    ...
    for (wf_name, wf_config) in wavefunction_types
        ...
        try
            ...
            @printf(
                "  Estimated energy: %.6f (%.6f per site)\n",
                final_energy,
                energy_per_site
            )
    ...
    println(
        "Ansatz             Params  Energy/site   Variance   Quality  Time(s)  Efficiency",
    )
    println("-" * 80)
    ...
    for wf_name in ["Simple Slater", "Slater + Jastrow", "RBM Enhanced", "Full Ansatz"]
        ...
        @printf(
            "%-18s %-7d %10.6f   %8.5f   %7.3f  %7.2f  %9.4f\n",
            wf_name,
            result[:parameters],
    ...
    @printf(
        "  Correlation contribution: %.4f per site (%.1f%%)\n",
        correlation_energy,
        100 * correlation_energy / simple_energy
    )
