...
function main()
    ...
    for (calc_name, calc_mode, n_samples) in calculation_types
        ...
        try
            ...
            @printf(
                "  Energy: %.6f ± %.6f\n",
                real(sim.energy_estimate),
                sqrt(sim.energy_variance)
            )
    ...
    for (calc_name, result) in simulation_results
        ...
        if haskey(result, :mock_data)
            ...
            energy_series = generate_correlated_series(
                n_samples,
                base_energy,
            ...
            @printf(
                "    Effective samples: %.0f / %d\n",
                effective_samples,
                length(energy_series)
            )
    ...
    for (calc_name, result) in simulation_results
        ...
        @printf(
            "    vs. Exact: %.6f (error: %.6f)\n",
            exact_energy_per_site,
            energy_per_site - exact_energy_per_site
        )
...
function calculate_autocorrelation(series, max_lag)
    ...
    for lag = 1:max_lag
        covariance =
            mean((series[1:(n-lag)] .- mean_val) .* (series[(lag+1):n] .- mean_val))
...
function perform_block_averaging(series)
    ...
    block_sizes = [2^i for i = 1:Int(floor(log2(max_block_size)))]
...
function estimate_double_occupancy(U, t, energy_per_site)
    ...
    return max(
        0.0,
        min(0.25, 0.1 + (energy_per_site - kinetic_scale) / potential_scale * 0.15),
    )
end
