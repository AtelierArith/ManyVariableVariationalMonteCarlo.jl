...
function main()
    ...
    for (name, config) in model_systems
        size_str =
            length(config[:size]) == 1 ? "$(config[:size][1])" :
            "$(config[:size][1])×$(config[:size][2])"
    ...
    for (model_name, model_config) in model_systems
        ...
        try
            if model_name == "1D Hubbard"
                ham, geom, ham_config =
                    stdface_chain(L, "Hubbard"; t = params[:t], U = params[:U])
            elseif model_name == "2D Hubbard"
                ham, geom, ham_config = stdface_square(
                    Lx,
                    Ly,
                    "Hubbard";
                    t = params[:t],
            ...
            elseif model_name == "2D J1-J2 Heisenberg"
                ham, geom, ham_config = stdface_triangular(Lx, Ly, "Spin"; J = params[:J])
                # Note: J2 would be added manually in full implementation
            elseif model_name == "1D Kondo"
                ham, geom, ham_config =
                    stdface_chain(L, "Kondo"; t = params[:t], J_K = params[:J_K])
            elseif model_name == "2D Attractive Hubbard"
                ham, geom, ham_config =
                    stdface_square(Lx, Ly, "Hubbard"; t = params[:t], U = params[:U])
            end
        ...
        println(
            "  Layout: Proj=$(layout.n_proj), Slater=$(layout.n_slater), " *
        ...
        try
            ...
            results[model_name] = Dict(
                ...
                :convergence_quality =>
                    energy_variance < 0.1 ? "Good" : "Needs improvement",
            )
    ...
    fermion_models =
        [name for name in keys(results) if "Hubbard" in name || "Kondo" in name]
    ...
    converged_models = [
        name for (name, result) in results if
        result[:convergence_quality] in ["Good", "Estimated"]
    ]
    ...
    difficult_models =
        [name for (name, result) in results if result[:complexity] in ["High", "Very High"]]
    ...
    @printf(
        "  • Average energy accuracy: %.1e per site\n",
