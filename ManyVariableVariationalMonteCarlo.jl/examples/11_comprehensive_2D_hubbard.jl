...
function main()
    ...
    println("\nLattice Analysis:")
    println("  Site 1 (corner): $(length(neighbors_nn[1])) NN, $(length(neighbors_nnn[1]))-$(length(neighbors_nn[1])) NNN")
    println("  Site 6 (edge): $(length(neighbors_nn[6])) NN, $(length(neighbors_nnn[6]))-$(length(neighbors_nn[6])) NNN") 
    println("  Site 10 (bulk): $(length(neighbors_nn[10])) NN, $(length(neighbors_nnn[10]))-$(length(neighbors_nn[10])) NNN")
    ...
    try
        test_energy = calculate_hamiltonian(ham, test_electron_config, test_electron_numbers)
    ...
    println("  - Hilbert space dimension: C($(Lx*Ly), $nelec) ≈ $(binomial(BigInt(Lx*Ly), BigInt(nelec)))")
