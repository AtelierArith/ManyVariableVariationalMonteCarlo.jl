...
function main()
    ...
    for (name, layout) in complexity_levels
        ...
        println("    Proj=$(layout.n_proj), Slater=$(layout.n_slater), " *
    ...
    for complexity_name in ["Simple", "Medium", "Complex"]
        ...
        for (method_name, method_config) in optimization_methods
            ...
            try
                ...
                @printf("  Final energy: %.6f (%.6f per site)\n", 
                       final_energy, final_energy_per_site)
                ...
                @printf("  Estimated final energy: %.6f (%.6f per site)\n", 
                       final_energy, final_energy / L)
        ...
        for method_name in sort(collect(keys(complexity_results)))
            ...
            @printf("%-20s %10.6f   %8.5f   %6.2f   %s\n",
                   method_name, result[:final_energy_per_site], 
                   result[:energy_variance], result[:optimization_time],
                   convergence_symbol)
        end
    end
    ...
    for complexity_name in ["Simple", "Medium", "Complex"]
        ...
        @printf("%-12s %-25s %10.6f   %8.3f\n",
               complexity_name, best_method, best_energy, best_efficiency)
    end
    ...
    for complexity_name in ["Simple", "Medium", "Complex"]
        ...
        @printf("%-11d  %7.2f    %7.2f    %9.2f   %s\n",
               n_params, sr_time, cg_time, adam_time, fastest)
    end
    ...
    for method_name in sort(collect(keys(optimization_methods)))
        ...
        for (complexity, result) in [
            ...
            @printf("    %-8s: variance = %.5f (%s)\n", 
                   complexity, result[:energy_variance], status)
        end
        ...
        # Efficiency analysis
        simple_eff = -simple_result[:final_energy_per_site] / simple_result[:optimization_time]
        complex_eff = -complex_result[:final_energy_per_site] / complex_result[:optimization_time]
        
        @printf("  Efficiency change: %.1f%% (simple → complex)\n", 
