@testitem "hamiltonian: APBC and twist phases on boundary wraps" begin
    ...
    ham_tw = create_hubbard_hamiltonian(n, ne, t, U; lattice_type=:chain, apbc=false, twist_x=pi/2)
    ...
    function get_coeff(ham, i, j, spin)
        for term in ham.transfer_terms
            if term.site_i == i && term.site_j == j && term.spin_i == spin && term.spin_j == spin
    ...
    n2 = 4
    ham_sq = create_hubbard_hamiltonian(n2, ne, t, U; lattice_type=:square, apbc=false, twist_x=pi/3, twist_y=pi/4)
