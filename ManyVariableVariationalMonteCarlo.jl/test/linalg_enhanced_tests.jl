@testitem "linalg_enhanced" begin
    """
    ...
    @testset "Pfaffian Enhanced Operations" begin
        ...
        # Test antisymmetric matrix creation with stable seed
        rng = StableRNG(12345)
        n = 6
        ...
        # Test Pfaffian calculation with error handling
        local pf_val
        try
            pf_val = pfaffian(A)
            ...
            U = randn(rng, ComplexF64, n, 2)
            V = randn(rng, ComplexF64, n, 2)

            pf_new2 = woodbury_update!(calc, U, V)
            @test isa(pf_new2, ComplexF64)
        catch e
            if isa(e, ManyVariableVariationalMonteCarlo.PfaffianLimitError)
                @test_skip "Pfaffian value below numerical limit (expected for some matrices)"
                return  # Skip the rest of the tests if matrix is ill-conditioned
            else
                rethrow(e)
            end
        end
