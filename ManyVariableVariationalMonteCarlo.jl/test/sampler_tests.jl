@testitem "VMCConfig basic functionality" begin
    ...
    config = VMCConfig(; n_samples=500, n_thermalization=50, n_measurement=50,
                       n_update_per_sample=2, acceptance_target=0.6, temperature=0.5,
                       use_two_electron_updates=true, two_electron_probability=0.2)
...
@testitem "VMCState move proposals" begin
    ...
    electron1_idx, new_pos1, old_pos1, electron2_idx, new_pos2, old_pos2 = propose_two_electron_move(state, rng)
...
@testitem "VMCResults basic functionality" begin
    ...
    observables = Dict("energy" => energy_samples, "magnetization" => [0.1+0.0im, 0.2+0.0im, 0.3+0.0im])
    results = VMCResults{ComplexF64}(1.5+0.0im, 0.5+0.0im, energy_samples, observables,
                                    0.6, 3, 10, 3, 1.0, 3)
...
@testitem "VMC sampling integration" begin
    ...
    initialize_vmc_state!(state, initial_positions)
    config = VMCConfig(; n_samples=10, n_thermalization=5, n_measurement=5,
                       n_update_per_sample=1, use_two_electron_updates=false)
    rng = MersenneTwister(12345)
    # Run VMC sampling
